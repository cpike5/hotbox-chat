...
{
    [McpServerTool]
    [Description("Sends a message to a text channel in HotBox. Requires the agent's bearer token.")]
    public static async Task<string> SendMessage(
        [Description("The GUID of the channel to send the message to")] Guid channelId,
        ...
        HotBoxApiClient? client = null)
    {
        ...
        try
        {
            var result = await client.SendMessageAsync(channelId, content, bearerToken);
